...
def create_text_features(words, max_words, we, we_dim):
    raw_text = ' '.join(words)
    words = [word for word in words if word in we.index_to_key]
        ...
        if strategy == 'clip':
            if feat_2d is None:
                ...
                if cur_n_tokens_2d != 0 and cur_n_tokens_3d != 0:
                    ...
                    #print("checking 2d feat shape used for getting the video",feat_2d[:n_tokens].shape)
                    #print("checking 3d feat shape used for getting the video",feat_3d[:n_tokens].shape)
